...
| **语音引擎** | `VoiceEngine` | 管理 KWS/ASR/TTS 模型，维护音频队列 |
...
graph TD
    ...
    DialogRepeat["↻ DialogRepeat<br/><i>SuccessIsRunning 装饰器</i><br/><small>将 SUCCESS 映射为 RUNNING<br/>实现无限对话循环</small>"]
    DialogLoop["⟶ DialogLoop<br/><i>Sequence(memory=False)</i><br/><small>每轮完成后自动重置</small>"]
    Listen["🎤 ListenCommand<br/>流式 ASR<br/><small>→ bb.user_command</small>"]
...
| **DialogRepeat** | SuccessIsRunning | - | 将 DialogLoop 的 SUCCESS 映射为 RUNNING，使对话持续循环 |
...
sequenceDiagram
    ...
    Note over Main: root.status == SUCCESS/FAILURE
...
stateDiagram-v2
    ...
    state DialogActive {
        [*] --> Listening
        Listening --> Listening: RUNNING (ASR 识别中)
        ...
        Speaking --> Listening: TTS 播放完成 → 下一轮对话
    }

    DialogActive --> Idle: 对话超时 / 回合结束\nroot.stop(INVALID)
```
...
| `DialogRepeat` | `SuccessIsRunning` 装饰器 | 将 `DialogLoop` 的 SUCCESS 转换为 RUNNING，实现无限对话循环 |
| `DialogLoop` | `Sequence(memory=False)` | 单轮对话流程：听→识别→执行→说。`memory=False` 每轮自动重置 |
...
| `ListenCommand` | `nodes/listen.py` | 流式 ASR 识别，检测到端点且有文本返回 SUCCESS | Write: `user_command`, `last_activity_time` |
...
flowchart TD
    ...
    C4 -->|"No → FAILURE"| DEF["DefaultResponse\n兜底 (always SUCCESS)"]
...
├── engine.py                        # VoiceEngine：管理 KWS/ASR/TTS 模型和音频流
├── config.py                        # RobotConfig：所有可配置参数
├── nodes/
│   ├── __init__.py                  # 节点模块导出
│   ├── wake_word.py                 # WaitForWakeWord - 唤醒词检测
│   ├── listen.py                    # ListenCommand - 流式 ASR
...
class RobotConfig:
    ...
    # 对话
    dialog_timeout: float = 15.0       # 无活动超时 (秒)
...
# 3. main.py - 在 DefaultResponse 之前添加
